...
### ★ 车型参数已经「打通」到 AI 生成（2026-09-24）

在管理面板里改车型参数（或下架某个车型），**AI 写稿立刻用新参数**，不用重新构建部署。

链路：打开内容生成页 / 每次点生成 → 先拉一次云端知识库 → 把 car 条目写进网页版的
虚拟目录 `/vault/wiki/car-model/` → `ai-adapter.mjs` 照常读这个目录。
也就是说**服务端的取参数逻辑一行没改**，只是它读到的内容变成云端最新那份。

几个行为要心里有数：

- 车型名会自动归一，「智己 L6 官方参数.md」和「智己 L6 官方参数（演示数据）」
  算同一台车，改了会覆盖而不是并存两条。
- **下架 = 从车型下拉里彻底消失**，生成时也选不到它（不是「还在但用旧参数」）。
- 云端接口挂了、或一条车型都没返回时，原样沿用构建期快照 —— 宁可用旧参数，
  也不会让全员没法生成。
- 目前只有**车型参数**这一类接进了 AI 路径；权益 / 创作知识 / 爆文库仍只供知识页展示。
...
它会在云端把 L6 的续航临时改成 777 km、确认浏览器读到了新值，再把它设成下架、
确认车型下拉从 7 个变 6 个，最后自动还原。
